...
const Carousel = lazy(() =>
  import("@/components").then((m) => ({ default: m.Carousel }))
) as typeof CarouselType;

export function PopularityAndLatestSection({
  initialData,
}: {
  initialData: PaginationEntity<ArticleSummary>;
}) {
  const [activeKey, setActiveKey] = useState<"POPULARITY" | "LATEST">(
    "POPULARITY"
  );

  const { data, status, carouselProps } =
    useInfiniteQueryWithCarousel<ArticleSummary>({
      queryKey: [
        activeKey === "POPULARITY"
          ? QUERY_KEYS.ARTICLE_LIST_POPULARITY
          : QUERY_KEYS.ARTICLE_LIST_LATEST,
      ],
      queryFn: ({ pageParam }) => {
...
function Success({ data }: { data: Array<ArticleSummary> }) {
  return data.map((v, i) => <ArticleCard key={i} {...v} isPriority={i < 2} />);
}
