import { RenderSdu, getDesignToken, Col } from "sdu-kit";
import type { SduType } from "sdu-kit";
import { RenderArticleContent, TypeArticleContent, articleTokens } from 'notion-article-kit';

function addDesignPrefix(sdu: Array<Object | string>, prefix?: string): Array<SduType | string> {
  return sdu.map((item: any) => {
    if(typeof item === 'object') {
      if('meta' in item) prefix = item.meta.design
      if((item.type as string).split('-').length === 1) item.type = `${prefix}-${item.type}`
      if('children' in item) item.children = addDesignPrefix(item.children, prefix)
    }
      return item
  }) as Array<SduType | string>
}
...
export default async function Home() {
  ...
  const tokens = await getDesignToken('default')
  const semantic = tokens.find((token) => token['semantic'] !== undefined)?.semantic

  const rawSdu: Array<Object | string> = [
    {
      meta: {
        design: 'default',
        theme: 'light'
      },
      type: 'headlineContainer',
      props: {},
      children: [
        {
          type: 'headlineSection',
          props: {headline: 'This is SDU test'},
          children: [
        {
          type: 'shared-grid',
          props: {
          ...
          children: [
            {
              ...
              props: {
                ...
                gap: 4,
                style: {
                  backgroundColor: semantic.containerEven,
                }
              },
              children: [
                {
                ...
                },
                {
                  ...
                  children: [
                    'this is the first text of internet'
                  ]
                }
              ]
            },
  ...
  const article = await fetch('http://localhost:4000/article/title3')
    .then(res => res.json())

  return (
    <>
      <RenderSdu ui={addDesignPrefix(rawSdu)} />
      <Col width="fill" alignItems="center" className={articleTokens}>
        <Col maxWidth={720}>
