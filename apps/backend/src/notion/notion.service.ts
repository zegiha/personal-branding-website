...
export class NotionService {
  ...
  // Partial 블록 타입 가드
  private isPartialBlock(block: BlockObjectResponse | PartialBlockObjectResponse): block is PartialBlockObjectResponse {
  ...
  async getPageById(id: string): Promise<PageObjectResponse> {
    try {
      ...
      // 응답 타입 검증
      if(!('parent' in res)) {
        throw new InternalServerErrorException('Response is not a valid page object');
      }
      ...
      if (isNotionClientError(error)) {
        switch (error.code) {
          ...
          default:
            ...
            throw new InternalServerErrorException(`Notion API error: ${error.code}`);
        }
      }
  ...
  async getDatabaseById(id: string): Promise<DatabaseObjectResponse> {
    try {
      ...
      // 응답 타입 검증
      if(!('parent' in res)) {
        throw new InternalServerErrorException('Response is not a valid database object');
      }
  ...
  async getPathUntilRoot(idCheck: Set<string>, parentId: string, parentType: 'page' | 'database'): Promise<string> {
    try {
      // 순환 참조 체크
      if(idCheck.has(parentId)) {
        throw new BadRequestException(`Circular reference detected in hierarchy: ${parentId}`);
      }
      idCheck.add(parentId)

      if(parentType === 'page') {
        const page = await this.getPageById(parentId)
        if(page.parent.type !== 'page_id' && page.parent.type !== 'database_id' && page.parent.type !== 'data_source_id') {
          return ''
        }
        const nextTitle = await this.getPathUntilRoot(
          idCheck,
          page.parent.type === 'page_id' ? page.parent.page_id : page.parent.database_id,
        ...
        if(database.parent.type !== 'page_id' && database.parent.type !== 'database_id') {
        ...
        const nextPath = await this.getPathUntilRoot(
          idCheck,
          database.parent.type === 'page_id' ? database.parent.page_id : database.parent.database_id,
  ...
  async getDatabase(): Promise<NotionDatabaseResponseDto[]> {
    try {
      ...
      if (!Array.isArray(res.results)) {
        throw new InternalServerErrorException('Invalid response format from Notion API');
      }

      const typeSafeDatasource = res.results as DataSourceObjectResponse[];

      const paths = await Promise.all(typeSafeDatasource.map((res) =>
      ...
      if (isNotionClientError(error)) {
        switch (error.code) {
          case APIErrorCode.Unauthorized:
            throw new BadRequestException('Unauthorized access to Notion workspace');
          case APIErrorCode.RateLimited:
            throw new InternalServerErrorException('Rate limit exceeded, please try again later');
          default:
  ...
  async getPage(id: string, pathPrefix: string): Promise<NotionPageResponseDto[]> {
    try {
      // 입력 검증
      if (!id || typeof id !== 'string') {
      ...
      .then(res => res.filter(res =>
        res.object === 'page' &&
        'properties' in res &&
        'Title' in res.properties && res.properties.Title.type === 'title' &&
        'Tag' in res.properties && res.properties.Tag.type === 'multi_select' &&
        'Id' in res.properties && res.properties.Id.type === 'unique_id'
      ))) as PageObjectResponse[];

      const pages = res.map((res) => ({
        id: res.id,
        title: (res.properties.Title as any).title.map(v => v.plain_text).join(''),
  ...
  private async getAllChildren(blockId: string): Promise<ListBlockChildrenResponse['results']> {
    try {
      let allResults: ListBlockChildrenResponse['results'] = [];
      ...
      const partialBlocks = allResults.filter(block => this.isPartialBlock(block));
      ...
      throw new InternalServerErrorException('Failed to retrieve block children');
    }
  }
