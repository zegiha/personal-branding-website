...
exports.parseNotionBlocksWithChildrenMap = parseNotionBlocksWithChildrenMap;
 ...
 */
function parseNotionBlocksWithChildrenMap(blocks, childMap) {
    // Create a map for quick block lookup by ID
    const blockMap = new Map(blocks.map((block) => [block.id, block]));
    // Find all block IDs that are children of other blocks
    const childBlockIds = new Set(Object.values(childMap).flat());
    const parseBlockRecursively = (block) => {
        ...
        const parsedChildren = childBlocks && childBlocks.length > 0
            ? childBlocks.map(parseBlockRecursively).filter((v) => v !== undefined)
            : undefined;
        // Get the basic parsed content
        const baseContent = (0, parseNotionBlockToArticleContent_1.parseNotionBlockToArticleContent)(block);
