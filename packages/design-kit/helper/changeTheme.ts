...
export async function changeTheme(theme: ThemeEnum) {
	...
	try {
		if (window != undefined) {
			...
			if (!html)
				throw new Error("window is not undefined, but html is undefined");
			...
			html.classList.remove(
				...Object.values(ThemeEnum).map((v) => `${v}-palette`),
			);
			html.classList.remove(
				...Object.values(ThemeEnum).map((v) => `${v}-radius`),
			);
			html.classList.remove(
				...Object.values(ThemeEnum).map((v) => `${v}-typography`),
			);
			html.classList.remove(
				...Object.values(ThemeEnum).map((v) => `${v}-${themeMode}`),
			);
			html.classList.add(
				`${theme}-palette`,
				`${theme}-radius`,
